// components/pdp-rich-editor-2/core.js
...
function inlineRunsToText(runs) {
  return Array.isArray(runs) ? runs.map((r) => String(r?.text || "")).join("") : "";
}
  ...
  if (block.type === "table") {
    ...
    return rows
      .map((row) => {
        ...
        return cells.map((cell) => inlineRunsToText(cell?.content)).join("	");
      })
      .join("\n");
  }

  return inlineRunsToText(block.content);
}
...
export function normalizeDoc(doc) {
  ...
  for (const blk of d.content) {
    if (!["p", "h", "li", "table", "code"].includes(blk.type)) blk.type = "p";
    ...
    if (blk.type === "table") {
      ...
      blk.theme = blk.theme === "plain" ? "plain" : "blue";
      blk.headerColor = normalizeColor(blk.headerColor) || "#dbeafe";
...
export function docToHTML(doc) {
  ...
  while (i < d.content.length) {
    ...
    if (b.type === "table") {
      ...
      const headerColor = normalizeColor(b.headerColor) || "#dbeafe";
      ...
      blocks.push(`<table class="re-content-table" data-theme="${theme}" data-header-color="${escapeAttr(headerColor)}" style="--re-table-header-bg:${escapeAttr(headerColor)};"><tbody>${rows}</tbody></table>`);
      i++;
      continue;
    }
