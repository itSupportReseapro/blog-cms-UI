// components/pdp-rich-editor/selection-dom.js

/**
 * Map DOM selection <-> absolute model offsets.
 *
 * IMPORTANT:
 * Model indexing = blockText + '
' + blockText + ...
 * Blocks: <p>, <h1>, <h2>, <h3>, <li>, and table cells with '	' / '
' separators.
 * Lists: each direct <li> is its own block.
 *
 * ALSO IMPORTANT:
 * The editor DOM may contain ZWSP (​) or NBSP ( ) as caret helpers.
 * These MUST NOT affect model offsets.
 */
const IGNORE_RE = /[\u200B\u00A0\uFEFF]/g;

function cleanText(s) {
  return String(s || "").replace(IGNORE_RE, "");
}

function cleanLen(s) {
  return cleanText(s).length;
}

export function getSelectionOffsets(rootEl) {
  const sel = window.getSelection();
  if (!sel || sel.rangeCount === 0) return { from: 0, to: 0 };

  const r = sel.getRangeAt(0);
  const from = pointToAbs(rootEl, r.startContainer, r.startOffset);
  const to = pointToAbs(rootEl, r.endContainer, r.endOffset);

  return normalize(from, to);
}

export function getRangeRectSafe(rootEl) {
  const sel = window.getSelection();
  if (!sel || sel.rangeCount === 0) return null;

  const r = sel.getRangeAt(0);
  if (!rootEl.contains(r.startContainer) || !rootEl.contains(r.endContainer)) return null;

  const rects = r.getClientRects();
  return rects && rects.length ? rects[0] : r.getBoundingClientRect();
}

/* ----------------- internals ----------------- */

function normalize(a, b) {
  let from = Number.isFinite(a) ? a : 0;
  let to = Number.isFinite(b) ? b : 0;
  if (from > to) [from, to] = [to, from];
  return { from, to };
}

function isAtomicNode(element) {
  return (
    element?.getAttribute?.("data-re-node") === "image" ||
    element?.getAttribute?.("data-re-node") === "equation"
  );
}

function isBlockNode(el) {
  if (isAtomicNode(el)) return true;
  const t = el?.tagName?.toLowerCase();
  return (
    t === "p" ||
    t === "h1" ||
    t === "h2" ||
    t === "h3" ||
    t === "pre" ||
    t === "li" ||
    t === "td" ||
    t === "th" ||
    t === "figure"
  );
}

function getSegments(rootEl) {
  const segments = [];
  const topChildren = Array.from(rootEl.children || []);

  for (let i = 0; i < topChildren.length; i++) {
    const child = topChildren[i];
    const tag = child.tagName?.toLowerCase();
    const isLastTopLevel = i === topChildren.length - 1;

    if (isAtomicNode(child)) {
      segments.push({
        node: child,
        length: 1,
        sepAfter: !isLastTopLevel ? 1 : 0,
        atomic: true,
      });
      continue;
    }

    if (tag === "ul" || tag === "ol") {
      const lis = Array.from(child.querySelectorAll(":scope > li"));
      for (let liIndex = 0; liIndex < lis.length; liIndex++) {
        const li = lis[liIndex];
        const isLastLi = liIndex === lis.length - 1;
        segments.push({
          node: li,
          length: getBlockTextLength(li),
          sepAfter: !isLastLi ? 1 : (!isLastTopLevel ? 1 : 0),
        });
      }
      continue;
    }

    if (tag === "table") {
      const rows = Array.from(child.querySelectorAll(":scope > tbody > tr, :scope > tr"));
      for (let ri = 0; ri < rows.length; ri++) {
        const cells = Array.from(rows[ri].querySelectorAll(":scope > td, :scope > th"));
        for (let ci = 0; ci < cells.length; ci++) {
          const isLastCell = ci === cells.length - 1;
          const isLastRow = ri === rows.length - 1;
          let sepAfter = 0;
          if (!isLastCell) sepAfter = 1; // tab
          else if (!isLastRow) sepAfter = 1; // row newline
          else if (!isLastTopLevel) sepAfter = 1; // block newline after table

          segments.push({
            node: cells[ci],
            length: getBlockTextLength(cells[ci]),
            sepAfter,
          });
        }
      }
      continue;
    }

    if (isBlockNode(child)) {
      segments.push({
        node: child,
        length: getBlockTextLength(child),
        sepAfter: !isLastTopLevel ? 1 : 0,
      });
    }
  }

  return segments;
}

function findContainingBlock(rootEl, node) {
  const segments = getSegments(rootEl);

  for (const segment of segments) {
    if (segment.node === node) {
      return segment.node;
    }

    if (segment.node?.contains?.(node)) {
      return segment.node;
    }
  }

  return null;
}

function getFirstBlockFromNode(node) {
  if (!node || node.nodeType !== Node.ELEMENT_NODE) return null;
  if (isBlockNode(node)) return node;

  const tag = node.tagName?.toLowerCase();
  if (tag === "ul" || tag === "ol") {
    return node.querySelector(":scope > li");
  }
  if (tag === "table") {
    return node.querySelector("td, th");
  }

  return node.querySelector("p, h1, h2, h3, pre, li, td, th, figure");
}

function getLastBlockFromNode(node) {
  if (!node || node.nodeType !== Node.ELEMENT_NODE) {
    return null;
  }

  if (isBlockNode(node)) {
    return node;
  }

  const tag = node.tagName?.toLowerCase();
  let matchingBlocks = [];

  if (tag === "ul" || tag === "ol") {
    matchingBlocks = Array.from(
      node.querySelectorAll(":scope > li")
    );
  } else if (tag === "table") {
    matchingBlocks = Array.from(
      node.querySelectorAll("td, th")
    );
  } else {
    matchingBlocks = Array.from(
      node.querySelectorAll(
        "p, h1, h2, h3, pre, li, td, th, figure"
      )
    );
  }

  return matchingBlocks[matchingBlocks.length - 1] || null;
}

function resolveRootPointBlock(rootEl, container, offset) {
  if (container !== rootEl || !Number.isFinite(offset)) return null;

  const childNodes = Array.from(rootEl.childNodes || []);
  if (!childNodes.length) return null;

  const idx = Math.max(0, Math.min(offset, childNodes.length));

  if (idx < childNodes.length) {
    const fromNext = getFirstBlockFromNode(childNodes[idx]);
    if (fromNext) return { blockEl: fromNext, innerOffset: 0 };
  }

  for (let i = idx - 1; i >= 0; i--) {
    const fromPrev = getLastBlockFromNode(childNodes[i]);
    if (fromPrev) {
      return { blockEl: fromPrev, innerOffset: getBlockTextLength(fromPrev) };
    }
  }

  return null;
}

function measureNodeClean(node) {
  if (!node) return 0;

  if (node.nodeType === Node.TEXT_NODE) {
    return cleanLen(node.nodeValue || "");
  }

  if (node.nodeType !== Node.ELEMENT_NODE) {
    return 0;
  }

  /*
   * An equation or image is one document position.
   * Never count the MathML text inside an equation.
   */
  if (isAtomicNode(node)) {
    return 1;
  }

  let length = 0;

  for (const child of node.childNodes || []) {
    length += measureNodeClean(child);
  }

  return length;
}

function getBlockTextLength(blockEl) {
  return measureNodeClean(blockEl);
}

function getOffsetWithinPointClean(
  blockEl,
  container,
  nodeOffset
) {
  if (isAtomicNode(blockEl)) {
    return nodeOffset <= 0 ? 0 : 1;
  }

  let total = 0;
  let found = false;

  const walk = (node) => {
    if (!node || found) return;

    if (node === container) {
      if (node.nodeType === Node.TEXT_NODE) {
        total += realOffsetToCleanOffset(
          node.nodeValue || "",
          nodeOffset
        );
      } else if (
        node.nodeType === Node.ELEMENT_NODE
      ) {
        if (isAtomicNode(node)) {
          total += nodeOffset <= 0 ? 0 : 1;
        } else {
          const endOffset = Math.max(
            0,
            Math.min(
              nodeOffset,
              node.childNodes.length
            )
          );

          for (
            let index = 0;
            index < endOffset;
            index++
          ) {
            total += measureNodeClean(
              node.childNodes[index]
            );
          }
        }
      }

      found = true;
      return;
    }

    if (node.nodeType === Node.TEXT_NODE) {
      total += cleanLen(node.nodeValue || "");
      return;
    }

    if (node.nodeType !== Node.ELEMENT_NODE) {
      return;
    }

    if (isAtomicNode(node)) {
      /*
       * A browser can sometimes report the endpoint
       * from inside contenteditable=false MathML.
       */
      if (node.contains?.(container)) {
        total += nodeOffset <= 0 ? 0 : 1;
        found = true;
      } else {
        total += 1;
      }

      return;
    }

    for (const child of node.childNodes || []) {
      walk(child);

      if (found) {
        return;
      }
    }
  };

  walk(blockEl);

  return total;
}

function pointToAbs(rootEl, container, offset) {
  const segments = getSegments(rootEl);
  if (!segments.length) return 0;

  let bEl = findContainingBlock(rootEl, container);
  let inner = 0;

  if (!bEl) {
    const rootPoint = resolveRootPointBlock(rootEl, container, offset);
    if (!rootPoint) return 0;
    bEl = rootPoint.blockEl;
    inner = rootPoint.innerOffset;
  }

  const segmentIndex = segments.findIndex((seg) => seg.node === bEl);
  if (segmentIndex < 0) return 0;

  if (container !== rootEl) {
    inner = getOffsetWithinPointClean(bEl, container, offset);
  }

  let abs = 0;
  for (let i = 0; i < segmentIndex; i++) {
    abs += segments[i].length + segments[i].sepAfter;
  }

  return abs + inner;
}

function realOffsetToCleanOffset(raw, realOffset) {
  const upto = String(raw || "").slice(0, Math.max(0, realOffset));
  return cleanLen(upto);
}

function cleanOffsetToRealOffset(raw, cleanOffset) {
  const s = String(raw || "");
  let cleanCount = 0;

  for (let i = 0; i <= s.length; i++) {
    if (cleanCount >= cleanOffset) return i;
    const ch = s[i];
    if (ch == null) break;
    if (ch === "​" || ch === " " || ch === "﻿") continue;
    cleanCount++;
  }

  return s.length;
}

export function setSelectionOffsets(rootEl, from, to) {
  const segments = getSegments(rootEl);
  if (!segments.length) return;

  const absToPoint = (abs) => {
    let pos = Math.max(0, Number.isFinite(abs) ? abs : 0);

    for (const seg of segments) {
      const len = seg.length;
      if (pos <= len) return { node: seg.node, innerOffset: pos, atomic: seg.atomic };

      pos -= len;
      if (seg.sepAfter) {
        if (pos <= seg.sepAfter) return { node: seg.node, innerOffset: len, atomic: seg.atomic };
        pos -= seg.sepAfter;
      }
    }

    const last = segments[segments.length - 1];
    return { node: last.node, innerOffset: last.length, atomic: last.atomic };
  };

  const resolveTextNodeAt = (
    blockEl,
    innerOffset,
    isAtomic = false
  ) => {
    if (isAtomic || isAtomicNode(blockEl)) {
      const parent =
        blockEl.parentNode || rootEl;

      const index = Array.from(
        parent.childNodes
      ).indexOf(blockEl);

      return {
        node: parent,
        offset:
          innerOffset <= 0
            ? Math.max(0, index)
            : Math.max(0, index + 1),
      };
    }

    const locatePoint = (
      parent,
      requestedOffset
    ) => {
      let remaining = Math.max(
        0,
        requestedOffset
      );

      const children = Array.from(
        parent.childNodes || []
      );

      for (
        let index = 0;
        index < children.length;
        index++
      ) {
        const child = children[index];

        if (
          child.nodeType === Node.ELEMENT_NODE &&
          isAtomicNode(child)
        ) {
          if (remaining === 0) {
            return {
              node: parent,
              offset: index,
            };
          }

          if (remaining <= 1) {
            return {
              node: parent,
              offset: index + 1,
            };
          }

          remaining -= 1;
          continue;
        }

        if (child.nodeType === Node.TEXT_NODE) {
          const raw = child.nodeValue || "";
          const length = cleanLen(raw);

          if (remaining <= length) {
            return {
              node: child,
              offset: cleanOffsetToRealOffset(
                raw,
                remaining
              ),
            };
          }

          remaining -= length;
          continue;
        }

        if (
          child.nodeType === Node.ELEMENT_NODE
        ) {
          const length =
            measureNodeClean(child);

          if (remaining <= length) {
            return locatePoint(
              child,
              remaining
            );
          }

          remaining -= length;
        }
      }

      return {
        node: parent,
        offset: children.length,
      };
    };

    return locatePoint(
      blockEl,
      innerOffset
    );
  };

  const a = absToPoint(from);
  const b = absToPoint(to);
  if (!a.node || !b.node) return;

  const pA = resolveTextNodeAt(a.node, a.innerOffset, a.atomic);
  const pB = resolveTextNodeAt(b.node, b.innerOffset, b.atomic);

  const sel = window.getSelection();
  if (!sel) return;

  const range = document.createRange();
  range.setStart(pA.node, pA.offset);
  range.setEnd(pB.node, pB.offset);

  sel.removeAllRanges();
  sel.addRange(range);
}
